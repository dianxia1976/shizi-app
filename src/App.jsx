...
// Try to load overrides from localStorage at module load so CHARS is correct from first render
function _initialOverrides() {
  try {
    const v = window.localStorage.getItem("shizi_overrides_v1");
    return v ? JSON.parse(v) : null;
  } catch(e) { return null; }
}
let CHARS = buildChars(_initialOverrides());
...
const LS_KEY_STATE = "shizi_state_v1";
const LS_KEY_OVERRIDES = "shizi_overrides_v1";
function lsGet(key) {
...
export default function App(){
  // Load saved state/overrides from localStorage on first render.
  // At this point CHARS was already initialized from overrides (see top of file).
  const[state,setState]=useState(() => {
    const saved = lsGet(LS_KEY_STATE);
    if (!saved) return mkState();
    // If the saved state has different char count than current CHARS, resize
    if (saved.chars && saved.chars.length !== CHARS.length) {
      const newChars = CHARS.map((_, i) => {
        if (saved.chars[i]) return {...saved.chars[i], idx: i};
        return {idx: i, st: "unknown", fam: 0, wc: 0, sc: 0};
      });
      return {...saved, chars: newChars};
    }
    return saved;
  });
  ...
  const[overrides,setOverrides]=useState(() => lsGet(LS_KEY_OVERRIDES));

  // Persist state changes to localStorage (debounced implicitly via useEffect)
  useEffect(() => { lsSet(LS_KEY_STATE, state); }, [state]);
  useEffect(() => { lsSet(LS_KEY_OVERRIDES, overrides); }, [overrides]);

  // Rebuild CHARS whenever overrides change, and resync state.chars
  useEffect(() => {
    CHARS = buildChars(overrides);
    setState(s => {
      if (s.chars.length === CHARS.length) return s;
      // Rebuild chars array to match new CHARS length, keeping progress at same indexes where possible
      const oldMap = {};
      s.chars.forEach((c, i) => { oldMap[i] = c; });
      const newChars = CHARS.map((_, i) => {
        if (oldMap[i]) return {...oldMap[i], idx: i};
        return {idx: i, st: "unknown", fam: 0, wc: 0, sc: 0};
      });
...
function SetPage({P}) {
  ...
  const exportData = () => {
    ...
    a.href = url;
    a.download = "shizi-backup-" + new Date().toISOString().slice(0,10) + ".json";
  ...
  const importData = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (!data.state) throw new Error("文件格式错误");
        if (!confirm("导入后将覆盖现有记录，确定继续？")) return;
        if (data.overrides) P.setOverrides(data.overrides);
  ...
  let storageStatus = "✅ 已启用";
  try {
    const testKey = "__shizi_test__";
    window.localStorage.setItem(testKey, "1");
    window.localStorage.removeItem(testKey);
  } catch(e) {
    storageStatus = "⚠️ 不可用（记录只保留到刷新）";
  }

  return (
    <div style={S.pg}>
      <Bk o={()=>P.setPage("home")}/>
      <div style={S.pt}>设置</div>

      <div style={S.card}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontWeight:600,fontSize:13}}>家长模式</span>
          <button onClick={()=>P.setState(s=>({...s,pm:!s.pm}))} style={{...S.tg,background:P.state.pm?"#60a5fa":"#cbd5e1"}}>
            <div style={{...S.td,transform:P.state.pm?"translateX(22px)":"translateX(2px)"}}/>
          </button>
        </div>
        <div style={{fontSize:11,color:"#94a3b8",marginTop:2}}>{P.state.pm?"显示拼音和课号":"孩子模式"}</div>
      </div>
      ...
      <div style={S.card}>
        <div style={{fontSize:12,fontWeight:600,marginBottom:6}}>📤 备份与恢复</div>
        <button onClick={exportData} style={{...S.btn,background:"linear-gradient(135deg,#4ade80,#22c55e)",fontSize:13,marginBottom:6}}>📥 导出记录（JSON）</button>
        <button onClick={()=>fileInputRef.current && fileInputRef.current.click()} style={{...S.btn,background:"linear-gradient(135deg,#60a5fa,#818cf8)",fontSize:13}}>📂 导入记录</button>
        <input type="file" ref={fileInputRef} onChange={importData} accept=".json" style={{display:"none"}} />
        <div style={{fontSize:10,color:"#94a3b8",marginTop:6,lineHeight:1.4}}>可在换设备、换浏览器时迁移数据</div>
      </div>
